...
const MAX_VALUE_SIZE: usize = 1024 * 1024 * 16;
...
impl SimpleDb {
    ...
    pub fn save_index_snapshot(&self) -> DbResult<()> {
        ...
        let (index_clone, current_log_offset, index_path) = {
            let inner = self
                ...
                .read()
                .map_err(|e| DbError::LockPoisoned(e.to_string()))?;
            let log_offset = inner.writer.buffer().len() as u64 + inner.last_sync_offset;
    ...
    pub fn compact(&self) -> DbResult<()> {
        ...
        fs::rename(&compaction_log_path, &original_log_path)
            .map_err(|e| DbError::CompactionError(format!("Failed to rename compacted log: {}", e)))?;
        info!("Renamed log: {:?} -> {:?}", compaction_log_path, original_log_path);

        fs::rename(&compaction_index_path, &original_index_path)
            .map_err(|e| DbError::CompactionError(format!("Failed to rename compacted index: {}", e)))?;
        ...
        let swap_result: DbResult<()> = {
            let mut inner = self.inner.write()
             ...
             if end_offset != compacted_log_offset {
                 warn!("Compaction: New log end offset {} does not match expected offset {}. Index might be slightly off.", end_offset, compacted_log_offset);
             }
...
mod tests {
    ...
    async fn test_compaction() {
        ...
        assert_eq!(db.get(key_c).unwrap(), None);
        ...
        assert_eq!(db.inner.read().unwrap().index.len(), 4);
    }
    ...
    async fn test_snapshotting() {
        ...
        let val_after = b"val_after";
        {
            ...
            let offset_before_extra_put = {
                let inner = db1.inner.read().unwrap();
                inner.writer.buffer().len() as u64 + inner.last_sync_offset
            };
            db1.save_index_snapshot().expect("Failed to save snapshot");
            let snapshot_read_offset = { // Re-read snapshot to check offset
             ...
             assert!(snapshot_read_offset >= offset_before_extra_put, "Snapshot offset should be at or after the put");
        ...
        assert_eq!(db2.get(key_after).expect("DB2 snap GET key_after failed"), Some(val_after.to_vec()));
